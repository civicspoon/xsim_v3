...
import { 
  ...
  BarChart3, ShieldCheck, Maximize2, X, Box, Search
} from "lucide-react";
...
function GalleryContent() {
  ...
  const [selectedImage, setSelectedImage] = useState(null); // สำหรับ Lightbox
  ...
  return (
    <div className="flex h-screen w-screen bg-[#020202] text-white overflow-hidden font-sans">
      ...
      {/* 📁 Sidebar */}
      <aside className="w-[350px] bg-white/[0.01] border-r border-white/5 backdrop-blur-3xl p-8 flex flex-col">
        <div className="mb-10">
          <div className="flex items-center gap-3 mb-2">
             ...
             <h2 className="text-2xl font-black tracking-tighter uppercase italic">Gallery</h2>
          </div>
        ...
        <nav className="flex-1 space-y-3 overflow-y-auto pr-2 custom-scrollbar">
          {data.categories.map((cat) => (
            <button
              ...
              className={`w-full group flex items-center justify-between p-5 rounded-[1.5rem] border transition-all duration-500 ${
                selectedCat?.id === cat.id 
                ? 'bg-red-600/10 border-red-600/50 shadow-[0_0_20px_rgba(220,38,38,0.1)]' 
                : 'bg-white/[0.02] border-white/5 hover:border-white/20'
              } ${cat.baggageCount === 0 ? 'opacity-50 cursor-default' : ''}`}
            >
      ...
      {/* 🖼️ Main Content Area (3 Columns Grid) */}
      <main className="flex-1 p-12 overflow-y-auto relative bg-[radial-gradient(circle_at_top_right,_var(--tw-gradient-stops))] from-red-900/5 via-transparent to-transparent">
        {selectedCat ? (
          <div className="animate-in fade-in duration-500">
            <div className="mb-10 flex justify-between items-end">
               <div>
                  <h3 className="text-6xl font-black tracking-tight uppercase mb-2">{selectedCat.name}</h3>
                  <div className="flex items-center gap-4">
            ...
            {loadingImages ? (
              <div className="flex justify-center py-20"><Loader2 className="w-12 h-12 text-red-600 animate-spin" /></div>
                ...
                /* ✅ ปรับเป็น 3 Columns Grid ที่นี่ */
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                   {baggageList.map((bag) => (
                     <div key={bag.id} onClick={() => setPreviewItem(bag)} className="group relative bg-white/[0.02] border border-white/5 rounded-[2.5rem] p-6 cursor-pointer hover:bg-white/[0.04] hover:border-red-600/40 transition-all duration-500">
        ...
        ) : <div className="h-full flex items-center justify-center opacity-10 font-black text-4xl uppercase tracking-[0.5em]">Select Category</div>}
      </main>

      {/* 🔍 Inspection Modal with Lightbox Logic */}
      {previewItem && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-8 backdrop-blur-3xl bg-black/90 animate-in fade-in duration-300">
          <div className="relative w-full max-w-7xl bg-[#080808] border border-white/10 rounded-[3rem] p-12 overflow-y-auto max-h-[95vh]">
            ...
            <div className="mb-12">
              ...
              <p className="text-gray-500 font-bold uppercase tracking-[0.2em] mt-2 text-xs">Object: {previewItem.item?.name}</p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {[
                ...
                <div key={idx} className="group/item">
                  ...
                  {/* ✅ คลิกที่รูปใน Modal เพื่อเปิด Lightbox */}
                  <div 
                    onClick={() => setSelectedImage({ src: view.src, label: view.label })}
                    className="aspect-square bg-black rounded-[2.5rem] border border-white/5 flex items-center justify-center p-6 cursor-zoom-in hover:border-red-600/50 transition-all"
                  >
                    <img src={getImageUrl(view.src)} className="w-full h-full object-contain" alt={view.label} />
                  </div>
                </div>
              ))}
            </div>
      ...
      {/* 🌌 Lightbox Layer (Full-screen view) */}
      {selectedImage && (
        <div 
          className="fixed inset-0 z-[200] bg-black flex flex-col items-center justify-center p-4 animate-in zoom-in duration-200"
        ...
        >
           ...
           <div className="max-w-[90vw] max-h-[80vh] flex items-center justify-center">
              <img 
                ...
                className="max-w-full max-h-full object-contain shadow-[0_0_50px_rgba(220,38,38,0.2)]" 
                alt="Fullscreen" 
              />
           </div>
